...
output_file_excel_name = 'Trial_1_Drug_6K0o' # DO NOT ADD THE EXTENSION AKA FILE TYPE!!!! ALSO REMEMBER TO CHANGE NUMBER BEFORE RUNNING!!!!!!
...
proteins = {
    ...
    'name' : "Iba1",
    'pdb_name' : '2D58',
    'pdb_file_name' : '2d58_minocycline.pdb'
    },
    'CD163': {
    'name' : "CD163",
    'pdb_name' : '6K0O',
    'pdb_file_name' : '6k0o_minocycline.pdb'
    },
}
start = time.time()
...
forcefield = ForceField('protein.ff14SB.xml', 'implicit/obc1.xml','Minocycline_FF.xml') # Force Fields used

protein_sim = PDBFile(pdbfile) # Imports PDB file to then simulate
system_drug = forcefield.createSystem(protein_sim.topology, nonbondedMethod=CutoffNonPeriodic, nonbondedCutoff=1*nanometer, constraints=HBonds) # Change these settings to change system, to make one easily go to https://github.com/openmm/openmm/blob/master/docs-source/usersguide/application/02_running_sims.rst#the-openmm-setup-application and follow the instructions there

    
simulation_drug = Simulation(protein_sim.topology, system_drug, integrator, platform,platformProperties)
simulation_drug.context.setPositions(protein_sim.positions)
simulation_drug.minimizeEnergy()

reporting_steps = 50000

simulation_drug.reporters.append(StateDataReporter(stdout, reporting_steps, step=True, speed =True, remainingTime = True, totalSteps = 100000000))

output_file = 'output_drug0_6K0o.pdb'
pdb_reporter = app.PDBReporter(output_file, reporting_steps)  # Starts the snapshots @ time = 0; starts at output_normal0_Iba1.pdb

simulation_drug.reporters.append(pdb_reporter)
simulation_drug.reporters.append(StateDataReporter(f'Trial_1_Drug_{protein['name']}.log', reporting_steps,  # Creates the .log file with those things to be tracked
                      step=True,
...
checkpoint_file = f'checkpoint_drug0_{protein['name']}.chk'
simulation_drug.reporters.append(CheckpointReporter(checkpoint_file, reporting_steps)) # Checkpoint every snapshot in case more info is needed
...
simulation_drug.context.setVelocitiesToTemperature(temperature)
simulation_drug.step(equilibrationSteps)

for step in range(10000):
    simulation_drug.step(10000)  # Perform 10000 steps at a time
    ...
    if step % 5 == 0:
        simulation_drug.reporters = [reporter for reporter in simulation_drug.reporters if not isinstance(reporter, PDBReporter)] # Removes old reporters
        simulation_drug.reporters = [reporter for reporter in simulation_drug.reporters if not isinstance(reporter, CheckpointReporter)] # Removes old reporters
        output_file = f"output_drug{file_count}_{protein['name']}.pdb"
        checkpoint_file = f'checkpoint_drug{file_count}_{protein['name']}.chk'
        simulation_drug.reporters.append(CheckpointReporter(checkpoint_file, reporting_steps))
        all_output_files.append(output_file)
        pdb_reporter = PDBReporter(output_file, reporting_steps)  # Create a new reporter
        simulation_drug.reporters.append(pdb_reporter)  # Append the new reporter
        file_count += 1
